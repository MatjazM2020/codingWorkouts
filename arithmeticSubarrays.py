...
class Solution:
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        ...
        for i in range(n):
            ...
            tempArr = nums[l[i]:r[i]+1]
            tempArr.sort()

            dif = tempArr[1] - tempArr[0]
            for j in range(1, len(tempArr)):
                if tempArr[j]-tempArr[j-1] != dif:
                    res[i] = False
                    break
        return res
...
Return a list of boolean elements answer, where answer[i] is true if the subarray nums[l[i]], nums[l[i]+1], ... , nums[r[i]] can be 
